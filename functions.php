...
function my_update_user_profile_endpoint() {
    register_rest_route( 'my-plugin/v1', '/update-user-profile', array(
        'methods' => 'POST',
        'callback' => 'my_update_user_profile_callback',
         
    ) );
}
add_action( 'rest_api_init', 'my_update_user_profile_endpoint' );

function my_update_user_profile_callback( $request ) {
       ...
       $user_data = array(
		...
		if($user_data['async-upload']['async-upload']){
				 ...
				 return  wp_send_json(
					array(
						...
						'data'    => array(
							...
							'filename' => esc_html($_FILES['async-upload']['async-upload']['name']),
						),
					)
				);
			}
			...
			if (isset($post_data['context']) && isset($post_data['theme'])) {
			  ...
			  $user_profile_url = $attachment['url'];
			  update_user_meta( $user_id, 'user_react_profile', $user_profile_url );
		}
...
function modify_pre_get_avatar_defaults($avatar, $id_or_email) { 

    // Update the $avatar variable according to your website requirements and return this variable. You can modify the $avatar variable conditionally too if you want.
	if($id_or_email != null){
		$user_profile_url = get_user_meta( $id_or_email, 'user_react_profile' , true );
		$avatar = '<img src="'.$user_profile_url.'" alt="User_profile" width="70"> ';

	}
    return $avatar; 
}
// add the filter
add_filter( "pre_get_avatar", "modify_pre_get_avatar_defaults", 10, 3 );  
